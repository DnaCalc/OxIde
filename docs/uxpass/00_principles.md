...
- Unless explicitly called out as a `tests/wtd/backfill.rs` journey, landed
  claims in this doc are treated as **unit-tested with end-to-end verification
  pending**.
- W045 is the active journey-backfill pass that promotes selected claims to
  release-binary verification.
- As of `2026-04-18`, the W045 backfill journeys plus a release-binary
  five-minute user pass verify: `Ctrl+O`, `Ctrl+N`, `Ctrl+S`,
  `Ctrl+Shift+S`, `Ctrl+Z/Ctrl+Y`, `F1` fallback/resolvable, `F12`
  fallback, `F5` run + `Esc` return, `F6` Enter dispatch, Empty-scene
  overlay backing, and overlay/run dirty-state preservation.
...
scene-fixed and never carries the marker. Unit-test coverage:
...
palette binding resolves to a wired `Msg` (J4-e / P6). Unit-test coverage:
...
Release-binary evidence now exists in `tests/wtd/backfill.rs` for
`Ctrl+S`, `Ctrl+Shift+S`, `Ctrl+Z/Ctrl+Y`, and dirty-state preservation
across overlay/run transitions, and those flows were re-checked in the
`2026-04-18` five-minute user pass.
...
The converse also holds, and unit-tested + release-binary WTD-verified (`tests/wtd/backfill.rs`): a binding that the
palette advertises must actually resolve to a wired `Msg`. `Create Project`
(`Ctrl+N`) is now wired end-to-end and remains in the palette command list
as a first-class affordance. Unit-test coverage:
`shell::state::tests::palette_create_project_is_wired_to_ctrl_n`.
